...
const getReceipts = async () => {
  return new Promise((resolve, reject) => {
    axios
      .get("https://3.130.4.139/graphql?query={receipts{amount,date_time}}")
      .then((res) => {
...
const getNPOofDay = async () => {
  return new Promise((resolve, reject) => {
    axios
      .get("https://3.130.4.139/graphql?query={NPOofDay{name,_id,at}}")
      .then((res) => {
...
const getAllNPOs = async () => {
  return new Promise((resolve, reject) => {
    axios
      .get("https://3.130.4.139/graphql?query={nonprofits{name,_id,total,at}}")
      .then((res) => {
...
const dailyNonprofitSelection = new cron.CronJob("0 0 * * *", async () => {
  ...
  for (var i = 0; i < receipts.length; i++) {
    ...
    if (receipts[i].date_time > yesterday && receipts[i].date_time < today) {
      console.log("TODAY: " + receipts[i].date_time + " " + receipts[i].amount);
